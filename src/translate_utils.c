...
int translate_num(long int* output, const char* str, long int lower_bound, 
    long int upper_bound) {
      ...
      //char* res;
      //*output = strtol(str, &res, 0);

      //check error: 1. cannot convert or empty str; 2. not all number; 3. exceed bounds
      //Q: consider 0?
      //if((*output == 0 && strlen(res) != 0) || strlen(res) != 0 || *output < lower_bound
              //|| *output > upper_bound){
          //*output = 0;
          //return -1;
      //}
  ...
  char* check;
    	//set base as 0 to handle all the possible formats
	*output = strtol(str,&check,0);
	
	if(check != str){//check for conversion happens or not
		
		if(*check != '\0'){ //Check for any invalide successive character
			*output = 0L; 
			return -1;	
		}else if((*output)>upper_bound || (*output)<lower_bound){//check for range
			return -1;
