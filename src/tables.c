...
SymbolTable* create_table(int mode) {
	...
	table->len = 0;
	table->tbl = (Symbol*)calloc(5,sizeof(Symbol)); 
	return table;
}
...
int add_to_table(SymbolTable* table, const char* name, uint32_t addr) {
	...
	//Checkfor word_alignment
	if(addr%4 != 0){
		addr_alignment_incorrect();
		return -1;
	}
	if(get_addr_for_symbol(table,name)!=-1 && table->mode == 1){
		name_already_exists(name);
		return -1;
	}
    	if(table->len< table->cap){//Not exceed the current capacity yet
		table->tbl[table->len].addr = addr;
		table->tbl[table->len].name = create_copy_of_str(name);
		table->len++;
		return 0;		
	}else{//Expand the symbol table and add the element
...
int64_t get_addr_for_symbol(SymbolTable* table, const char* name) {
    /* YOUR CODE HERE */
	// Return -1 if the table is NULL or empty or name is NULL
...
void write_table(SymbolTable* table, FILE* output) {
	/* YOUR CODE HERE */
	if(!table || !output){//Null argument check
