...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    ...
    while(fgets(buf, BUF_SIZE, input)) {
       input_line++;
        // Ignore comments
       skip_comment(buf);
	    if(*buf == '\0'){//the line is comment
		    continue;
	    }
        // Scan for the instruction name or label
	    char* token = strtok(buf, IGNORE_CHARS);
           if(!token){
        	 continue;
           }
	    int add_label_result = add_if_label(input_line,token,byte_offset,symtbl);
	    if(add_label_result){//the token represents label
		    if(add_label_result==1){
			    token = strtok(NULL,IGNORE_CHARS);
				//keep checking for labels
